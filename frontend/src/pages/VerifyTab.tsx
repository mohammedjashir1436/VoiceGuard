...
> = {
  'voiceguard-generated': {
    ...
    blurb:
      'The keyed spectral watermark matches — this clip was synthesised by VoiceGuard.',
  },
  ...
  'ai-generated': {
    ...
    blurb:
      'A signed C2PA manifest marks this audio as trained-algorithmic media.',
  },
  ...
  unknown: {
    ...
    blurb:
      'The file carries a C2PA manifest but no AI-generation assertion.',
  },
  ...
  'no-provenance-found': {
    ...
    blurb:
      'No watermark or manifest detected. This does NOT prove the audio is real — most recordings and most deepfakes carry no provenance marks. Use the Detect tab for a verdict.',
  },
}
...
function ResultPanel({
  ...
  const rows: { k: string; v: string }[] = [
...

    {
      ...
      v: result.c2pa_has_manifest
        ? `present · ${
            result.c2pa_validation_state ?? 'unvalidated'
          }`
        : 'absent',
    },
  ]
  ...
  if (result.c2pa_has_manifest) {
    rows.push({
      ...
      v: result.c2pa_ai_generated
        ? `AI-generated · ${
            result.c2pa_software_agent ?? 'unknown agent'
          }`
  ...
  return (
    <div className="space-y-4">
      <div
        className={`rounded-xl border-2 bg-gray-800 p-6 ${view.cls}`}
      >
...
export default function VerifyTab() {
  ...
  const [result, setResult] =
    useState<WatermarkVerifyResult | null>(null)
  ...
  const handleVerify = async () => {
    ...
    try {
      setResult(
        await verifyProvenance(file, watermarkId),
      )
    } catch (e) {
      if (e instanceof ApiError) {
        ...
        setError(
          'Could not reach the verification API. Please try again in a moment.',
        )
      }
    } finally {
      setLoading(false)
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400">
          The read side of Generate: check an audio file for
          VoiceGuard&apos;s spectral watermark and embedded C2PA
          manifest. Files are auto-deleted after 60s (PDPL).
        </p>
      </div>
        ...
        {file ? (
          ...
          <div className="space-y-2">
            ...
            <p className="text-gray-500 text-sm">
              e.g. a clip downloaded from the Generate tab
            </p>
          </div>
        )}
      </div>
      ...
      <div>
        <label
          ...
          htmlFor="vg-watermark-id"
        >
          Watermark ID{' '}
          <span className="text-gray-600">
            (optional — shown after generating)
          </span>
        </label>
