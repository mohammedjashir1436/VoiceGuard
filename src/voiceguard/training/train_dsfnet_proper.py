...
def get_model(
    ...
    Important:
    AASIST is imported only when an AASIST model is actually requested.
    This keeps the Wav2Vec2 training path independent of the old
    torchaudio-dependent AASIST implementation.
    """
    ...
    if model_name == "aasist_large":
        from voiceguard.models.aasist import AASISTLarge
...
def train(args: argparse.Namespace) -> None:
    ...
    if len(train_ds) == 0 or len(val_ds) == 0:
        print(
            "ERROR: train or val dataset empty",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(
        f"Model: {args.model} | Parameters: "
        f"{model.count_parameters():,}"
    )

    # Experiment label
    eff_bs = args.batch_size * max(
        1,
        args.accum_steps,
    )
    ...
    # Warm-up for 5% of epochs then cosine decay.
    warmup_epochs = max(
        1,
        int(0.05 * args.epochs),
    )
    ...
    # so resume doesn't clobber model_best.pt.
    resumed = bool(args.resume and ckpts)
    ...
    for epoch in range(
        ...
        for step, (waveform, labels) in enumerate(
            train_loader
        ):
            waveform = waveform.to(device)
        ...
        # Select best checkpoint on val_EER.
        # Test set is never used for selection.
        val_eer_curr = val_metrics["eer"]
        val_loss_curr = val_metrics["loss"]

        if (
            math.isnan(val_eer_curr)
            or math.isnan(val_loss_curr)
        ):
            print(
                f"WARNING: val metric is NaN "
                f"at epoch {epoch} — skipping",
                flush=True,
            )
            ...
            try:
                from voiceguard.models.checkpoint_manager import (
                    save_snapshot,
                )
                ...
                # Snapshot failure never interrupts training.
                pass
        ...
        if (
            ...
            torch.save(
                ckpt,
                out_dir
                / f"model_epoch{epoch:03d}.pt",
            )

        if (
            args.early_stop > 0
            and patience_counter >= args.early_stop
        ):
            print(
                f"Early stopping at epoch {epoch} "
                f"(best val_eer={best_val_eer:.4f} "
                f"@ epoch {best_epoch})"
            )

            break

    # Evaluate on test set once using the best
    # val_EER checkpoint — never used for selection.
    test_eer = float("nan")

    if (
        test_loader
        and (out_dir / "model_best.pt").exists()
    ):
        print(
