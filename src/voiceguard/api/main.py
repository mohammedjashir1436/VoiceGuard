...
def _detect_ssl_array(
    ...
    Always use the verified Hugging Face wav2vec2 anti-spoofing model
    for live microphone detection.
    """
    try:
        label, confidence = _detect_hf_array(
            audio,
            sr,
            "wav2vec2_spoof",
        )
        ...
        logger.info(
            "LIVE DETECTION: model=wav2vec2_spoof label=%s confidence=%.4f",
            label,
            confidence,
        )

        return label, confidence, "wav2vec2_spoof"

    except Exception:
        logger.exception("Live wav2vec2 detection failed")

        # Do not silently report REAL when the detector fails.
        return "unknown", 0.0, "wav2vec2_spoof"    """Score a raw audio window with the production detector (live streaming).
